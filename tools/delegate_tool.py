...
_DESCRIPTION_HEAD = (
    ...
    "Waits for every child by default; use their results before reaching the parent conclusion. Set background=true "
    "only for independent work that does not inform the current answer. Background dispatch returns live transcript "
    "paths immediately, then one completion per call by default; with delegation.independent_completions, each "
    "ungrouped task / `group` returns separately. Background results arrive only BETWEEN turns: finish independent "
    "work, give a one-line status, and END YOUR TURN. Never poll transcripts, artifacts, or CI for background children. "
