...
3. Create individual opportunity records for each company/role/watch target under `projects/job-opportunities/`, even if some are broad company boards rather than single postings. For broad boards, list the relevant roles discovered and set `status: researched` or `captured` with role-specific form inspection pending.
...
5. Cross-link the objective and records from `domains/opportunities`, relevant domain maps such as AI/physics/coding, `projects/project-backlog`, `projects/job-opportunities/README`, `_meta/index`, and `_meta/log`.
