...
- If Brayan wants latest upstream Hermes plus latest config schema, respect the local checkout workflow: the runtime personalization branch is `brayan/personal-hermes-customizations`. Keep `updates.branch: brayan/personal-hermes-customizations` in live config so both terminal `hermes update` and gateway `/update` stay on the integration branch. Prefer the daily documented fetch/rebase/test/push/detached-activation path; do not manually switch/update `main`.
