...
## Required reads
Read:
1. `~/personal-vault/decisions/pending.md`
2. `~/personal-vault/projects/dashboard.md`
...
- In cron/no-user contexts, avoid terminal heredoc verifier creation such as `python3 - <<'PY' ... PY`; approval policy may block it before the verifier can run. Safer default: create the `/tmp/hermes-verify-*.py` verifier with `write_file`, run it by path with `python3 /tmp/hermes-verify-...py`, have the verifier unlink itself, and print an explicit absence check in the same shell command.
