...
After adding a dated `## Review notes`, `## Cadence review notes`, or `## Review log` entry to the vault project hub, run `python3 ~/.hermes/scripts/project_review_history_retention.py --project <README-path> --keep 5`. Keep only the five newest dated review entries in the hub; durable decisions, outcomes, and evidence must be promoted into canonical fields/sections rather than depending on old rollover notes.
