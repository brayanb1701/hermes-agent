...
6. When this agent edits durable vault files, treat Markdown structure checks as ad-hoc verification, not canonical suite green. If no canonical lint/test exists, create a focused temporary verifier under `/tmp` with a `hermes-verify-` filename prefix, check the inserted recommendation/log section for uniqueness, expected evidence strings, required headings, updated frontmatter, and trailing whitespace, then clean the temp file up when possible. If normal cleanup is blocked by cron approval rules, a self-removing Python temp script is an acceptable cleanup pattern; report this explicitly as ad-hoc verification.
