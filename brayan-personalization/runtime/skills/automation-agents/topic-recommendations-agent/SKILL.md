...
6. When this agent edits durable vault files, treat Markdown structure checks as ad-hoc verification, not canonical suite green. If no canonical lint/test exists, create a focused temporary verifier under `/tmp` using Python `tempfile`/`mkstemp` or equivalent OS-safe naming with a `hermes-verify-` filename prefix; check the inserted recommendation/log section for uniqueness, expected evidence strings, required headings, updated frontmatter, scoped `git diff --check`, and trailing whitespace. Make expected-string checks match the exact wording actually inserted into both `queries/topic-recommendations.md` and `_meta/log.md`; if the verifier fails because the log summary uses different but acceptable wording, either adjust the log to include the key durable evidence phrase or adjust the verifier to the intended wording, then rerun until it passes. Clean the temp file up and explicitly verify/report cleanup (for example, no `/tmp/hermes-verify-*.py` leftovers). Avoid hard-coded temp verifier paths that can linger and trigger post-run verification warnings. In cron/no-user contexts, do **not** use `execute_code` for this verifier because arbitrary Python execution may be approval-blocked; instead write the verifier with `write_file` to `/tmp/hermes-verify-*.py`, run it via `terminal`, and make it self-remove in a `finally` block. Report this explicitly as ad-hoc verification, not suite green.
