...
- Target branch: `second-computer-evolution`
- Fork origin: `git@github.com:brayanb1701/hermes-agent.git`
- Official upstream: `git@github.com:NousResearch/hermes-agent.git`
- Workflow doc: `~/personal-vault/_meta/workflows/hermes/hermes-fork-update-workflow.md`
- Pre-run wake-gate script: `~/.hermes/scripts/hermes_upstream_rebase_ci.py`
  ...
  - After a verified candidate is pushed, the script schedules a detached transient unit that runs `hermes update --branch second-computer-evolution --yes --no-backup`; this activates the candidate, refreshes dependencies/config, and restarts the gateway.
  - If the live checkout was moved to `main` by an older/raw update path, a clean checkout is recovered from `origin/second-computer-evolution` instead of failing preflight.
...
- If the script wakes at `stage: preflight` because live `second-computer-evolution` and `origin/second-computer-evolution` diverged, inspect live/origin/worktree ancestry and source-only diffs rather than choosing by recency. Repair and verify the isolated worktree candidate, then finalize it directly with `--repo /home/brayan/.hermes/worktrees/hermes-upstream-rebase-ci --apply`; the finalizer pushes and schedules live activation. Do not move the live branch before candidate verification/remote publication. The historical references remain useful for ancestry/equivalence diagnosis, but their manual `git update-ref` activation steps are superseded by the detached updater.
