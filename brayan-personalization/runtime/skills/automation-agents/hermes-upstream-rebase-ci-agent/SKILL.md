...
- When upstream grouped background completion conflicts with the foreground-default delegation patches, preserve both contracts: wait for all children by default; explicit `background=true` reports ungrouped tasks individually and grouped tasks together. Keep descriptions within the schema budget and document grouping only for background mode. In `_run_children_parallel`, preserve upstream unit-local remaining counts (`n_here - len(results)`) alongside non-blocking interruption cleanup (`executor.shutdown(wait=not abandon_workers, cancel_futures=False)`). Run `scripts/run_tests.sh tests/tools/test_delegate*.py tests/tools/test_async_delegation*.py` plus the intake/cron suites; do not assume a guessed grouping-test filename exists. Record resolutions through rerere before continuing the rebase.
