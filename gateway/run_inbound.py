...
class GatewayInboundMixin:
    ...
    async def _enrich_inbound_images(
        ...
        from gateway.run import (
            enrich_anything_inbox_image, is_anything_inbox_source,
    ...
    async def _transcribe_one_clip(self, path: str, transcribe_audio, transcribe_audio_local_fallback) -> Tuple[Optional[str], str, Optional[str]]:
        """``(transcript_or_None, note, provider)`` for one clip via configured STT with local fallback."""
        ...
        if not result["success"]:
            ...
            return None, self._untranscribed_audio_note(path), None
        ...
        return transcript, f'"{transcript}"', result.get("provider")
    ...
    async def _enrich_message_with_transcription(
        ...
        for path in audio_paths:
            try:
                ...
                transcript, note, provider = await self._transcribe_one_clip(
                    path, transcribe_audio, transcribe_audio_local_fallback,
                )
                if transcript is not None:
                    successful_transcripts.append(transcript)
                    from gateway.run import (
                        is_anything_inbox_source, load_notes_intake_settings,
                    ...
                    if source and is_anything_inbox_source(source) and load_notes_intake_settings().enabled:
                        note = persist_audio_transcript(
                            user_text, path, transcript, provider=provider,
                        ).context_block
                enriched_parts.append(note)
            except Exception as e:
