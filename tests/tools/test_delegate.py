...
class TestDispatchDelegateTask(unittest.TestCase):
    ...
    def test_foreground_batch_interrupt_does_not_join_wedged_children(self):
        ...
        def run_delegate():
            with (
                ...
                patch("tools.delegate_tool._run_single_child", side_effect=wedged_child),
            ):
    ...
    def test_foreground_batch_interrupt_does_not_cancel_queued_children(self):
        ...
        # Keep the runner patch active until the queued daemon worker enters it;
        # the refactored Batch resolves _run_single_child lazily per worker.
        with (
            patch(
                "tools.delegate_tool._build_child_preserving_parent_tools",
                side_effect=children,
            ),
            patch("tools.delegate_tool._run_single_child", side_effect=child_run),
            patch(
